...
void PIO_Cfg(PIN_DESC cfg)
{
	...
	if(port_num <= PD_LAST_PORT)
	{
		...
		SYSCTL->SYSCTL_RCGCGPIO |= port_num;
		SYSCTL->SYSCTL_DCGCGPIO |= port_num; // SysCtlPeripheralDeepSleepEnable
		SYSCTL->SYSCTL_SCGCGPIO |= port_num; // SysCtlPeripheralSleepEnable

		// Unlock the commit register
		if(cfg & PD_LOCK)
		{
			port_base->GPIOLOCK = GPIO_GPIOLOCK_KEY;
...
void PIO_Free(PIN_DESC cfg)
{
	...
	if(port_num <= PD_LAST_PORT)
	{
		...
		if(!port_base->GPIOIM)
		{
			NVIC_DisableIRQ(GPIOPortA_IRQn + port_num);
		}
	}
}
...
void PIO_Write(PIN_DESC pins, unsigned int val)
{
	...
	if (port_num <= PD_LAST_PORT)
	{
		GPIO_TypeDef* port_base = PORT_ADDRESS( port_num );
		ASSERT(SYSCTL->SYSCTL_RCGCGPIO & (1<<port_num));
