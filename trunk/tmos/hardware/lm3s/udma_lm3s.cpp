...
//
//*****************************************************************************
void UDMA_Type::uDMAEnable(void)
{
    // Set the master enable bit in the config register.
   this->CFG = UDMA_CFG_MASTEN;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMADisable(void)
{
    this->CFG = 0;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAErrorStatusClear(void)
{
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelEnable(unsigned long ulChannel)
{
    ASSERT(ulChannel < 32);
    // Set the bit for this channel in the enable set register.
    this->ENASET = 1 << ulChannel;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelDisable(unsigned long ulChannel)
{
    ASSERT(ulChannel < 32);
    // Set the bit for this channel in the enable clear register.
    this->ENACLR = 1 << ulChannel;
}
...
int UDMA_Type::uDMAChannelIsEnabled(unsigned long ulChannel)
{
    ...
    return((this->ENASET & (1 << ulChannel)) ? true : false);
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAControlBaseSet(void *pControlTable)
{
    ...
    this->CTLBASE = (unsigned long)pControlTable;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelRequest(unsigned long ulChannel)
{
    ASSERT(ulChannel < 32);
    // Set the bit for this channel in the software uDMA request register.
    this->SWREQ = 1 << ulChannel;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelAttributeEnable(unsigned long ulChannel, unsigned long ulAttr)
{
    ASSERT(ulChannel < 32);
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelAttributeDisable(unsigned long ulChannel, unsigned long ulAttr)
{
    ASSERT(ulChannel < 32);
    ASSERT((ulAttr & ~(UDMA_ATTR_USEBURST | UDMA_ATTR_ALTSELECT |
                       UDMA_ATTR_HIGH_PRIORITY | UDMA_ATTR_REQMASK)) == 0);
    ...
    if(ulAttr & UDMA_ATTR_USEBURST)
        this->USEBURSTCLR = 1 << ulChannel;
    // Clear the alternate control select bit for this channel, if set in ulConfig.
    if(ulAttr & UDMA_ATTR_ALTSELECT)
        this->ALTCLR = 1 << ulChannel;
    // Clear the high priority bit for this channel, if set in ulConfig.
    if(ulAttr & UDMA_ATTR_HIGH_PRIORITY)
        this->PRIOCLR = 1 << ulChannel;
    // Clear the request mask bit for this channel, if set in ulConfig.
    if(ulAttr & UDMA_ATTR_REQMASK)
        this->REQMASKCLR = 1 << ulChannel;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelControlSet(unsigned long ulChannel, unsigned long ulControl)
{
	UDMA_CTRL_Type *pCtl;
    ASSERT(ulChannel < 64);
    ASSERT(this->CTLBASE != 0);
    // Get the base address of the control table.
    pCtl = (UDMA_CTRL_Type *)this->CTLBASE;
    // Get the current control word value and mask off the fields to be changed, then OR in the new settings.
    pCtl[ulChannel].CHCTL = ((pCtl[ulChannel].CHCTL &
                                  ~(UDMA_CHCTL_DSTINC_M |
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelTransferSet(unsigned long ulChannel, unsigned long ulMode,
                       void *pvSrcAddr, void *pvDstAddr,
                       unsigned long ulTransferSize)
{
	UDMA_CTRL_Type *pControlTable;
    ...
    // Get the current control word value and mask off the mode and size fields.
    ulControl = (pControlTable[ulChannel].CHCTL &
    ...
    // Set the transfer size and mode in the control word
    //(but don't write the control word yet as it could kick off a transfer).
    ...
    // increment is set to none, then the ending address is the same as the beginning.
    ...
    pControlTable[ulChannel].SRCENDP = (uint32_t)pvSrcAddr;
    ...
    // destination increment is set to none, then the ending address is the same as the beginning.
    ...
    pControlTable[ulChannel].DSTENDP = (uint32_t)pvDstAddr;
    // Write the new control word value.
    pControlTable[ulChannel].CHCTL = ulControl;
}
...
//
//*****************************************************************************
unsigned long UDMA_Type::uDMAChannelSizeGet(unsigned long ulChannel)
{
	UDMA_CTRL_Type *pControlTable;
    unsigned long ulControl;
    ASSERT(ulChannel < 64);
...
/** uDMAChannelDestPtrGet
 *
 * @param ulChannel
 */
void * UDMA_Type::uDMAChannelDestPtrGet(unsigned long ulChannel)
{
	UDMA_CTRL_Type *pControlTable;
    unsigned long ulControl;
    ASSERT(ulChannel < 64);
...
//
//*****************************************************************************
unsigned long UDMA_Type::uDMAChannelModeGet(unsigned long ulChannel)
{
	UDMA_CTRL_Type *pControlTable;
    unsigned long ulControl;
    ASSERT(ulChannel < 64);
    ...
    ulControl = pControlTable[ulChannel].CHCTL & UDMA_CHCTL_XFERMODE_M;
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelSelectSecondary(unsigned long ulSecPeriphs)
{
    // Select the secondary peripheral for the specified channels.
    this->CHALT |= ulSecPeriphs;
}
...
//
//*****************************************************************************
void UDMA_Type::uDMAChannelSelectDefault(unsigned long ulDefPeriphs)
{
    // Select the default peripheral for the specified channels.
    this->CHALT &= ~ulDefPeriphs;
}
