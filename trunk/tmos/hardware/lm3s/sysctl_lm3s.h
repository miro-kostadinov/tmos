...
struct SYSCTL_Type
{
    ...
    __IO uint32_t SRCRRegs[3];
    };
         uint8_t  RESERVED3[4];		//!< doc!
...
#define GPIOHSCTL GPIOHBCTL         //!< [__IO] GPIO High Speed Control
    ...
    __IO uint32_t RCGCRegs[3];
    };
         uint8_t  RESERVED8[4];		//!< doc!
    union{
    struct{
    ...
    __IO uint32_t SCGCRegs[3];
	};
         uint8_t  RESERVED9[4];		//!< doc!
   union{
	struct {
	...
	__IO uint32_t DCGCRegs[3];
   };
         uint8_t  RESERVED10[24];	//!< doc!
    __IO uint32_t DSLPCLKCFG;       //!< Deep Sleep-mode clock config reg
         uint8_t  RESERVED11[8];	//!< doc!
    ...
    void SysCtlPeripheralClockGating(int bEnable);
    ...
    void SysCtlIOSCVerificationSet(int bEnable);
    void SysCtlMOSCVerificationSet(int bEnable);
    void SysCtlPLLVerificationSet(int bEnable);
...
//
//	The following functions are outside of SYSCTL in order to reduce the calling code size

void SysCtlPeripheralEnable(unsigned long ulPeripheral);		//It is recommended to use Reset() instead when the peripheral is enabled for the first time
