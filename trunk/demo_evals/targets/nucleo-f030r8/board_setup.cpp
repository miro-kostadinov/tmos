...
static void SetSysClockTo48(void)
{
	...
	do
	{
		HSIStatus = RCC->RCC_CR & RCC_CR_HSIRDY;
		StartUpCounter++;
	...
	if (HSIStatus == 0x01)
	{
		...
		FLASH->FLASH_ACR |= FLASH_ACR_LATENCY_1WS;
...
//		RCC->RCC_CFGR |= RCC_CFGR_PPRE1_DIV2;
		...
		RCC->RCC_CFGR &=  ~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE | RCC_CFGR_PLLMUL);
		RCC->RCC_CFGR |= (RCC_CFGR_PLLMUL);
#endif /* STM32F10X_CL */
...
//     return;
	...
	system_clock_frequency = 48000000; //SYSCTL->SysCtlClockGet();
