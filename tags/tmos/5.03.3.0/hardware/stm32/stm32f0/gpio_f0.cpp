...
void PIO_Cfg(PIN_DESC cfg)
{
	...
	{
		RCC->RCC_AHBENR |= RCC_AHBENR_IOPAEN << tmpreg;
		...
		while(pin_pattern)
		{
			...
			/* Change mode */
			tmpreg = gpio_port->GPIO_MODER;
			tmpreg &= ~(0x3 << pos);
			tmpreg |= mode << pos;
			gpio_port->GPIO_MODER = tmpreg;

			/* Change pullup/pulldowns */
			tmpreg = gpio_port->GPIO_PUPDR;
			tmpreg &= ~(0x3 << pos);
			tmpreg |= PD_PULL_Get(cfg) << pos;
			gpio_port->GPIO_PUPDR = tmpreg;

			if(mode == PD_MODE_Get(PD_MODE_OUTPUT) || mode == PD_MODE_Get(PD_MODE_AF))
			{
				/* Change speed */
				tmpreg = gpio_port->GPIO_OSPEEDR;
				tmpreg &= ~(0x3 << pos);
				tmpreg |= PD_OSPEED_Get(cfg) << pos;
				gpio_port->GPIO_OSPEEDR = tmpreg;

				pos /= 2;

				/* Change output type */
				tmpreg = gpio_port->GPIO_OTYPER;
				tmpreg &= ~(0x1 << pos);
				tmpreg |= PD_OTYPE_Get(cfg) << pos;
				gpio_port->GPIO_OTYPER = tmpreg;
			}
		}
	}
