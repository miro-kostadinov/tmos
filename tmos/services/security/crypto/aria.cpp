...
const cipher_info_t aria_cipher_info =
{
   "ARIA",
   ARIA_BLOCK_SIZE,
   CIPHER_TYPE_BLOCK
...
{
   REV(0x517CC1B7), REV(0x27220A94), REV(0xFE13ABE8), REV(0xFA9A6EE0),
   REV(0x6DB14ACC), REV(0x9E21C820), REV(0xFF28B1D5), REV(0xEF5DE2B0),
   REV(0xDB92371D), REV(0x2126E970), REV(0x03249775), REV(0x04E8C90E)
};
...
cipher_algo_t* new_cipher_aria()
{
	...
	algo = new aria_cipher_algo_t(&aria_cipher_info);
	return algo;
}
...
RES_CODE aria_cipher_algo_t::init_key(const uint8_t* key, size_t key_size)
{
	uint32_t i;
	const uint32_t *ck1;
	const uint32_t *ck2;
...
void aria_cipher_algo_t::encrypt(const void* input, void* output, size_t blocks)
{
	while(blocks--)
	{
		uint32_t p[4];
		...
		input = (const uint8_t*)input + ARIA_BLOCK_SIZE;
		output = (uint8_t*)output + ARIA_BLOCK_SIZE;
	}
}
void aria_cipher_algo_t::decrypt(const void* input, void* output, size_t blocks)
{
	while(blocks--)
	{
		uint32_t p[4];
