...
//Macro definition
#define CLEAR_WORD32(a, i, n) memset((a)->mpi_data + i, 0, n * MPI_INT_SIZE);
#define COPY_WORD32(a, i, b, j, n) memcpy((a)->mpi_data + i, (b)->mpi_data + j, n * MPI_INT_SIZE);
...
RES_CODE secp128r1Mod(Mpi *a, const Mpi *p)
{
   ...
   res = a->mpiGrow(32 / MPI_INT_SIZE);
   if(res == RES_OK)
   {
	   res = t.mpiGrow(32 / MPI_INT_SIZE);
	   if(res == RES_OK)
	   {
...
RES_CODE secp160k1Mod(Mpi *a, const Mpi *p)
{
   ...
   //Ajust the size of the integers
   res = a->mpiGrow(40 / MPI_INT_SIZE);
   if(res == RES_OK)
   {
	   res = t.mpiGrow(24 / MPI_INT_SIZE);
	   if(res == RES_OK)
	   {
		   //Perform modular reduction
		   do
		   {
		      ...
		      //Compute T = T >> 32
		      res = t.mpiShiftRight(32);
		      if(res != RES_OK)
		    	  break;

		      //Compute A = A + (21389 * T)
		      res = t.mpiMulInt(&t, 21389);
		      if(res != RES_OK)
		    	  break;
		      res = a->mpiAdd(a, &t);
		      if(res != RES_OK)
...
RES_CODE secp192k1Mod(Mpi *a, const Mpi *p)
{
   ...
   //Ajust the size of the integers
   res = a->mpiGrow(48 / MPI_INT_SIZE);
   if(res == RES_OK)
   {
	   res = t.mpiGrow(28 / MPI_INT_SIZE);
	   if(res == RES_OK)
	   {
		   //Perform modular reduction
		   do
		   ...
		   if(res == RES_OK)
		   {
			   ...
			   if(res == RES_OK)
			   {
				   ...
				   if(res == RES_OK)
				   {
					   ...
					   while((res == RES_OK) && (t.mpiComp(p) >= 0))
					   {
					      res = t.mpiSub(&t, p);
					   }
			   ...
			   if(res == RES_OK)
			   {
				   ...
				   if(res == RES_OK)
				   {
					   ...
					   if(res == RES_OK)
					   {
						   ...
						   while((res == RES_OK) && (t.mpiCompInt(0) < 0))
						   {
						      res = t.mpiAdd(&t, p);
						   }
...
RES_CODE secp256k1Mod(Mpi *a, const Mpi *p)
{
   ...
   //Ajust the size of the integers
   res = a->mpiGrow(64 / MPI_INT_SIZE);
   if(res == RES_OK)
   {
	   res = t.mpiGrow(36 / MPI_INT_SIZE);
	   if(res == RES_OK)
	   {
		   //Perform modular reduction
		   do
...
RES_CODE secp384r1Mod(Mpi *a, const Mpi *p)
{
   ...
   //Ajust the size of the integers
   res = a->mpiGrow(96 / MPI_INT_SIZE);
   if(res == RES_OK)
   {
	   res = s.mpiGrow(48 / MPI_INT_SIZE);
	   if(res == RES_OK)
	   {
		   res = t.mpiGrow(48 / MPI_INT_SIZE);
		   if(res == RES_OK)
		   {
...
RES_CODE secp521r1Mod(Mpi *a, const Mpi *p)
{
   ...
   //Ajust the size of the integers
   res = a->mpiGrow(132 / MPI_INT_SIZE);
   if(res == RES_OK)
   {
	   res = t.mpiGrow(68 / MPI_INT_SIZE);
	   if(res == RES_OK)
	   {
		   //Compute A0
		   COPY_WORD32(&t, 0, a, 0, 17);
		   t.mpi_data[16] &= 0x000001FF;

		   //Compute A1
		   res = a->mpiShiftRight(521);
		   if(res == RES_OK)
		   {
			   //Compute A0 + A1
			   res = a->mpiAdd(a, &t);
			   ...
			   //Compute (A0 + A1) mod p
			   while((res == RES_OK) && (a->mpiComp(p) >= 0))
			   {
			      res = a->mpiSub(a, p);
			   }
		   }
