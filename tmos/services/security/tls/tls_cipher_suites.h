...
#ifndef SERVICES_SECURITY_TLS_TLS_CIPHER_SUITES_H_
#define SERVICES_SECURITY_TLS_TLS_CIPHER_SUITES_H_

#include <tmos.h>
#include <crypto.h>
#include <tls_opt.h>

enum tls_suite_id_t: uint16_t
{
...
struct tls_chipher_suite_info
{
	const char* 		suite_name;
	tls_suite_id_t		suite_identifier;
	tls_key_exch_t 		suite_key_exch;
	cipher_mode_t 		cipher_mode;
	new_cipher_algo_f 	new_cipher_algo;
	new_hash_algo_f 	new_hash_algo;
	new_hash_algo_f 	new_prf_hash_algo;
	uint8_t 			mac_key_len;
	uint8_t 			enc_key_len;
	uint8_t 			fixedIV_len;
	uint8_t 			recordIV_len;
	uint8_t 			auth_tag_len;
	uint8_t 			verify_data_len;
};
